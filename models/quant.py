...
class VectorQuantizer2(nn.Module):
    ...
    def get_next_autoregressive_input_inpaint(self, si: int, SN: int, f_hat: torch.Tensor, h_BChw: torch.Tensor, f_hats, mask_in, mask_out): # only used in VAR inference
        HW = self.v_patch_nums[-1]
        if si != SN-1:
            h = self.quant_resi[si/(SN-1)](F.interpolate(h_BChw, size=(HW, HW), mode='bicubic'))     # conv after upsample
